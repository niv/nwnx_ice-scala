package es.elv.kobold {
	...
	trait ActionQueue {
		...
		def doLock(what: G[_] with Lock) = assign {
	...
	trait Movement extends Position with ActionQueue {
		...
		def walkFrom(from: G[_], range: Double) =
			assign { R.proxy.actionMoveAwayFromObject(from, false, range) }
		def runFrom(from: G[_], range: Double) =
			assign { R.proxy.actionMoveAwayFromObject(from, true, range) }

		def follow(toFollow: G[_], followDistance: Double) = assign {
	...
	trait Trap {
		...
		def trapDetected(by: G[_] with Creature) = R.proxy.getTrapDetectedBy(this, by)
		def trapDetected(by: G[_] with Creature, status: Boolean) = R.proxy.setTrapDetectedBy(this, by, status)
	...
	trait SpellCasting extends ActionQueue {
		...
		def doCounterSpell(o: G[_]) = assign {
