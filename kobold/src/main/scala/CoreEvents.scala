			...
			e match {
				case r: RawEvent => {
					val e: Event = (r.event match {
						...
						case "player_login" => EventSource send new OnPlayerEnter(R.proxy.getEnteringObject)
						case "player_logout" => EventSource send new OnPlayerLeave(R.proxy.getExitingObject)
